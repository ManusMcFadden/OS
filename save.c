...
void run_command(char *buf, int nbuf, int *pcp) {
    ...
    if (pipe_cmd) {
        ...
        if (strcmp(arguments[0], "cd") == 0) {
            ...
            // Output the directory it is trying to change to
...
int main(void) {
    ...
    while (getcmd(buf, sizeof(buf)) >= 0) {
        printf("Entering command processing loop with command: %s\n", buf); // Debug output
        if (fork() == 0) {
            ...
            if (wait(&status) == 2) {
                ...
                close(pcp[0]);               // Close the read end after reading

                printf("Attempting to change directory to: %s\n", temp); // Debug output
                if (chdir(temp) < 0) {
                    printf("Error: chdir failed for %s\n", temp);
                } else {
                    printf("Successfully changed directory to: %s\n", temp); // Debug output
                }
                pipe(pcp);                   // Reopen the pipe for the next command
            }
        }
    }
    exit(0);
